...
# never collide with a real token id at the point of comparison; distinct from the
# ignore index -100 (written only later in __getitem__) for clarity.
GOLDFISH_TOKEN_ID = -2
...
# per dataloader worker. The modality LUTs live in
# megatron.core.tokenizers.utils.modality_lut with the same per-process pattern.
_GOLDFISH_CACHE = {}
