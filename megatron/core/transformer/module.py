...
def mark_keep_in_fp32(tensor: torch.Tensor) -> torch.Tensor:
    ...
    FP16/BF16 (e.g. the mHC / Manifold-Constrained Hyper-Connection mapping weights,
    whose mixing-weight computation is numerically sensitive and runs in FP32).
...
def convert_module_to_dtype_except_fp32_marked(
    ...
    Tensors marked with :func:`mark_keep_in_fp32` are left untouched. When no tensor
    is marked this is behaviourally identical to ``module.to(dtype)`` /
    ``module.half()`` / ``module.bfloat16()``.
...
class Float16Module(MegatronModule):
    ...
    def __init__(self, config: TransformerConfig, module: torch.nn.Module):
        ...
        if self.fp16:
            # convert_module_to_dtype_except_fp32_marked keeps mark_keep_in_fp32
            # tensors (e.g. mHC mapping weights) in FP32; identical to module.half()
            # when nothing is marked.
            self.add_module('module', convert_module_to_dtype_except_fp32_marked(module, torch.half))
