...
class InferenceAllGatherDispatcherBase(MoEAllGatherTokenDispatcher):
    ...
    These stable-address buffers are process-global. Only one active dynamic MoE
    inference context is supported per process; constructing another context
    replaces the provider state and the older context must not be used again.
    """
