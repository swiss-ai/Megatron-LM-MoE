...
#
# Ported from NVIDIA/Megatron-LM `dev` (v0.19) into this fork (v0.17), native
# path only: the cuTile/cuDNN fused kernels (``use_fused_mhc``), CUDA-graph
# capture hooks and selective-recompute machinery from upstream are intentionally
# dropped. The native torch path here is numerically identical to the reference
# and runs on CPU. See megatron/core/transformer/README-mhc.md-equivalent notes
# in the porting commit for what was trimmed.
...
class HyperConnectionModule(MegatronModule):
    ...
    def __init__(self, config: TransformerConfig, layer_number: int):
        ...
        # fused_add_3 always uses torch.compile (native_fused_add_3).
