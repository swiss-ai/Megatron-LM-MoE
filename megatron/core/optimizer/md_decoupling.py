...
class _MDDecouplingBase(torch.optim.Optimizer):
    ...
    def _param_step(self, p, group):
        ...
        if group["use_orthogonal_updates"]:  # Muon branch.
            ...
            with emerging_optimizers.utils.fp32_matmul_precision(self.fp32_matmul_prec):
                update = self._orthogonalize_param(p, grad, is_qkv=is_qkv, is_merged_offload_expert=is_merged_offload_expert)
    ...
    def _orthogonalize_param(self, p, grad, is_qkv: bool = False, is_merged_offload_expert: bool = False):
        ...
        if is_merged_offload_expert:
            ...
            for i, expert in enumerate(experts):
                experts[i] = self._orthogonalize_tensor(expert, tp_group, partition_dim)
...
class MDDecoupling(_MDDecouplingBase):
    ...
    def _preprocess_gains(self, p):
        ...
        # Undo gains to recover bare normalized weight. The *_bk slice tuples broadcast the gains
        # onto p: 2D (out, in) uses [:, None]/[None, :]; 3D (E, out, in) adds a leading expert axis.
        if flat_eff is not None:
            p.div_(flat_eff[layout["flat_bk"]].clamp_min(eps))
        if row_eff is not None:
            p.div_(row_eff[layout["row_bk"]].clamp_min(eps))
        if col_eff is not None:
            p.div_(col_eff[layout["col_bk"]].clamp_min(eps))
