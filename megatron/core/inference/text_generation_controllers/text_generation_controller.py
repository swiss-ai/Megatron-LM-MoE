...
class TextGenerationController:
    ...
    def _init_dynamic_sampling_tensors(self):
        ...
        if self._sampling_backend == "flashinfer":
            ...
            self._sampling = TorchSampling(self.sampling_rng, self.vocab_size)
    ...
    def _dynamic_step_context_init(
        ...
        # Configure the first inference MoE dispatcher to synchronize metadata.
    ...
    def _dummy_serial_mtp_forward(self):
        ...
        # Use a minimal SP-compatible size on every PP rank so ranks without
        # local MTP layers still issue a shape-matched pipeline broadcast.
    ...
    def _dynamic_step_context_bookkeeping(self) -> Dict[str, Tensor]:
        ...
        if getattr(context.kv_block_allocator, "block_routing", False) and finished_idxs.numel() > 0:
    ...
    async def async_generate_output_tokens_dynamic_batch(
        ...
        with torch.inference_mode():
            ...
            if getattr(context.kv_block_allocator, "block_routing", False):
