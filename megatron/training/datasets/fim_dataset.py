...
class GPTFIMDataset(GPTDataset):
    ...
    def _query_document_sample_shuffle_indices(self, idx: int) -> Tuple[np.ndarray, np.ndarray, list]:
        ...
        Returns:
            Tuple[np.ndarray, np.ndarray, list]: The text ids, document ids, and
                per-document token counts. FIM permutes segments across original
                document boundaries, so those boundaries are no longer meaningful
                afterwards -- the whole sample is reported as a single "document".
        """
