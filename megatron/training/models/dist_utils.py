    ...
    if (pg_collection.dp.rank() == 0) and (pg_collection.cp.rank() == 0):
        print(
            " > number of parameters on (tensor, gtp_remat, pipeline) model parallel rank ({}, {}, {}): {}".format(
                ...
                get_pg_rank(getattr(pg_collection, "gtp_remat", None)),
...
def _wrap_with_mp_wrapper(
    ...
    if (fp16 or bf16) and mixed_precision_wrapper is not None:
        # Honor the module precision contract before a recursive BF16/FP16 cast.
        # Restoring float() afterwards cannot recover truncated decay parameters.
        preserved = []
        for model_module in model_list:
            for submodule in model_module.modules():
                if hasattr(submodule, "_maintain_float32_expert_bias"):
                    for name in ("expert_bias", "qb_beta"):
                        buffer = getattr(submodule, name, None)
                        if buffer is not None:
                            preserved.append((submodule, name, buffer.data.float().clone()))
                for name in getattr(submodule, "_keep_in_float32_parameter_names", ()):
                    parameter = getattr(submodule, name)
                    preserved.append((submodule, name, parameter.data.float().clone()))
        model_list = [mixed_precision_wrapper(transformer_config, model_module) for model_module in model_list]
        for submodule, name, data in preserved:
            parameter = getattr(submodule, name)
            parameter.data = data.to(device=parameter.device)
