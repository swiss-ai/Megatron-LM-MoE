...
class TestTextGenerationController(TextGenerationControllerTestBase):
    ...
    def test_sample_from_dynamic_logits(
        ...
        context.request_query_lengths[:batch_size].fill_(1)
    ...
    def test_speculative_verify_tokens(self):
        ...
        # Override the backend entry point to return predictable speculative samples.
        self.text_generation_controller._sampling.sample_speculative = mock.MagicMock(
    ...
    def test_speculative_multinomial_sampling(self):
        ...
        # Configure metadata that forces multinomial sampling (top_p = 0.9, top_k = 0).
        ctx.active_request_metadata["temperature"][:2].fill_(1.0)
        ctx.active_request_metadata["top_k"][:2].fill_(0)
        ctx.active_request_metadata["top_p"][:2].fill_(0.9)
    ...
    def test_mtp_sp_padding_real_ranks(self, active_request_count):
        ...
        ctx.active_request_metadata["temperature"][:active_request_count].fill_(1.0)
        ctx.active_request_metadata["top_k"][:active_request_count].fill_(1)
        ctx.active_request_metadata["top_p"][:active_request_count].fill_(0.0)
