...
from megatron.training.training import _pipeline_shape_args, get_train_valid_test_num_samples
...
class TestEvalDataLoaderBatchSize:
    """Test that validation and test samplers use eval batch sizes."""

    @pytest.mark.parametrize(
        ("split", "expected_micro_batch_size"),
        [
            pytest.param(Split.train, 4, id="train"),
            pytest.param(Split.valid, 2, id="valid"),
            pytest.param(Split.test, 2, id="test"),
        ],
    )
    def test_split_selects_batch_size(self, monkeypatch, split, expected_micro_batch_size):
        class SplitDataset:
        ...
        monkeypatch.setattr(data_samplers.mpu, "get_data_parallel_world_size", lambda: 1)

        data_loader = data_samplers.build_pretraining_data_loader(SplitDataset(split), 0)

        assert data_loader.batch_sampler.micro_batch_size == expected_micro_batch_size


class TestEvalPipelineShape:
    """Test that evaluation uses its own micro batch size for pipeline buffers."""

    @pytest.mark.parametrize(
        ("is_packed", "expected"),
        [
            pytest.param(False, (128, 2), id="standard"),
            pytest.param(True, (256, 1), id="packed"),
        ],
    )
    def test_eval_micro_batch_size_controls_pipeline_shape(self, is_packed, expected):
        args = SimpleNamespace(
            seq_length=128,
            micro_batch_size=4,
            dataloader_inter_document_masking=is_packed,
            sft=False,
        )

        assert _pipeline_shape_args(args, micro_batch_size=2) == expected
